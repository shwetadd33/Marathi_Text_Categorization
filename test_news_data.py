...
#----------------------------------------------TF-IDF calculation------------------------------------------------------------
    ...
    for word ,calfre in relativefreqs.items():
        if word in inversefreq.keys():
            ...
            #if tfidffreqs[word]>0.0000 :
            test_vector_space[word] = tfidffreqs[word]

    #sorted_x = sorted(tfidffreqs.items(), key=operator.itemgetter(1))
    #print(sorted_x)
    #print(tfidffreqs)

#sorted_x = sorted(test_vector_space.items(), key=operator.itemgetter(1))
#print(sorted_x)
